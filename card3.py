...
card_sprites = []
cardlist =[[20,50],[40,50],[60,50],[80,50],[100,50]]
max_cards=5
# Load the card images and create card sprite objects

filenames = [f for f in os.listdir(path) if f.endswith('.png')]
for i, filename in enumerate(filenames[:max_cards]):
    card_sprite = Card("suit", i+1, os.path.join(path, filename))
    card_sprite.getSuit()
    print(card_sprite)
    x, y = cardlist[i]  # Get the position from the coordinate list
    card_sprite.move(x, y)  # Set the position of each card sprite
...
while loop:
  ...
  for event in pygame.event.get():
    ...
    for i in range(max_cards):
        screen.blit(card_sprites[i].image, card_sprites[i].rect)       # Update the display
    pygame.display.flip()
